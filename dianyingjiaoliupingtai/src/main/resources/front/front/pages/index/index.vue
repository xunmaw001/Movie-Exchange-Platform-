...
<view class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
      :style='{"backgroundColor":"var(--publicSubColor)","borderColor":"var(--publicMainColor)","color":"rgba(255, 255, 255, 1)","borderRadius":0,"borderWidth":"2rpx","fontSize":"32rpx","lineHeight":"72rpx","borderStyle":"solid"}'>
...
<view class="pl15 col3 header-title" style="padding: 0 12px;box-sizing: border-box;"
    ...
    <view class="right-content" @tap="onPageTap('dianying')"
          :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
        查看更多
    </view>
</view>
<view v-if='2 == 2' class="uni-product-list" :class="2 == 2 ? 'active' : ''" :style='{"borderRadius":"0","backgroundColor":"#efefef"}'>
    <view @tap="onDetailTap('dianying',product.id)" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in dianyingList" :key="index">
        <view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"var(--publicMainColor)","textAlign":"center"}' class="uni-product-title">{{product.dianyingName}}</view>
        <view class="image-view" :style='{"borderRadius":"2rpx","height":"180rpx"}'>
            <image :style='{"height":"180rpx","borderRadius":"90rpx"}' mode="aspectFill" class="uni-product-image" :src="baseUrl+product.dianyingPhoto"></image>
        </view>
    </view>
</view>
        </view>
...
<script>
    ...
    export default {
        ...
        async onShow() {
            ...
            let dianying = await this.$api.requestCondition('dianying','gexingtuijian',dianyingVal);

            this.dianyingList = dianying.data.list
            this.dianyingList.forEach(function(item, index) {
                if(item.dianyingContent != null && item.dianyingContent != "" && item.dianyingContent != "null"){
                    item.dianyingContent =item.dianyingContent.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi,"");//替换图片
                }
            });
