				...
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
					<view class="xiangqing">
						<view class="shangpinxiangqing">
							...
							<view class="app">
								{{detail.dianyingCollectionValue}}
							</view>
						</view>
					</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dianyingCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dianyingCollection', this.dianyingCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dianyingCollectionPhoto ? this.detail.dianyingCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dianyingCollectionYuyue', {
									yonghuId: _this.user.id,
									dianyingCollectionId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/dianyingCollectionYuyue/list');
							}
						}
					});
				},
