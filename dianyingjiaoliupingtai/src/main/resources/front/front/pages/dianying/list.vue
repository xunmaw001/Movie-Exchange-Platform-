	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 0px 0px rgba(0, 0, 0, 0.16)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"0 180rpx 180rpx 0","borderWidth":"4rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}'
						v-model="searchForm.dianyingName" type="text" placeholder="电影名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('dianying','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('dianying','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='dianyingTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in dianyingTypesList" :key="index" :class="dianyingTypes===item.codeIndex?'tabActive':''"
                                  @tap="dianyingTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <block v-for="(product,index) in list" :key="index">
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item"
            ...
            <image
                    ...
                    mode="aspectFill" :src="product.dianyingPhoto?baseUrl+product.dianyingPhoto.split(',')[0]:''">
            </image>
            <view class="list-item-body" style="flex: 1;">
                <view
                        ...
                        class="title hide1">{{product.dianyingName}}
                </view>
                <view v-if="product.dianyingNewMoney"
                      ...
                      class="title hide1">￥{{product.dianyingNewMoney}}
                </view>
                <view v-else-if="product.dianyingValue"
                      :style='{"padding":"10rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'
                      class="title hide1">{{product.dianyingValue}}
                </view>
                <view style="display: flex;justify-content: space-between;padding: 0 10rpx">
                    <text v-if="isAuth('dianying','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('dianying','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 电影类型 字段 字典表查询方法
            async dianyingTypesSelect(){
                let dianyingTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'dianying_types',
                });
                dianyingTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.dianyingTypesList = dianyingTypes.data.list
            },
            //当前表的 电影类型字段类别搜索
			dianyingTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.dianyingTypes != 0) {
                    params.dianyingTypes = this.dianyingTypes
                }
                let _this = this
                let res = await this.$api.list(`dianying`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('dianying', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.dianyingShenheTime=shijian;
                            await _this.$api.update(`dianying`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.dianyingName) {
                    searchForm['dianyingName'] = this.searchForm.dianyingName
                }

                let res = await this.$api.list(`dianying`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.dianyingContent!=null){
                        item.dianyingContent = item.dianyingContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
