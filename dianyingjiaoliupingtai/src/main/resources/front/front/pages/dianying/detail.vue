		...
		<view class="">
			<view class="price ">
				...
				<view class="pricejiner"
					  :style="detail.dianyingNewMoney>1000? 'width: 450rpx;':detail.dianyingNewMoney>10000? 'width: 550rpx;':'width: 400rpx;'">
					<text style='font-size:20rpx'>¥</text>
					<text style="font-weight:bold;font-size:50rpx">{{detail.dianyingNewMoney}}</text>
				</view>
			</view>
			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.dianyingName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.dianyingVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.dianyingContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dianyingId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getdianyingCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        dianyingId: this.dianyingId,
                        yonghuId: this.user.yonghuId,
						dianyingLiuyanPingfenNumber: this.scoreData,
                        dianyingLiuyanText: this.commentData,
                    }
                    await this.$api.save(`dianyingLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dianying', this.dianyingId);
                    this.detail = res.data
					this.detail.dianyingContent = this.detail.dianyingContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.dianyingPhoto ? this.detail.dianyingPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getdianyingCollection() {
					let params = {
						page: 1,
						limit: 1,
						dianyingId: this.dianyingId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`dianyingCollection`, params);
					if (res.data.list.length == 1) {
						let dianyingCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('dianyingCollection', JSON.stringify([dianyingCollectionId]));
									_this.$utils.msg('取消成功');
									_this.getdianyingCollection();
								}
							}
						});
						return;
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dianyingCollection', {
									yonghuId: _this.user.id,
									dianyingId: _this.detail.id,
									dianyingCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('dianyingLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						dianyingId: this.dianyingId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('dianying', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dianyingYuyue', {
									yonghuId: _this.user.id,
									dianyingId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/dianyingYuyue/list');
							}
						}
					});
				},
