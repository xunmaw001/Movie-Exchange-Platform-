        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.dianyingLiuyanText' placeholder="留言内容"></textarea>
            </view>
...
<script>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            let dianyingData = await this.$api.page(`dianying`, dianyingParams);
            this.dianyingOptions = dianyingData.data.list;
            let yonghuParams = {
                page: 1,
                limit: 100,
            }
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`dianyingLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.dianyingId){
                this.ruleForm.dianyingId = options.dianyingId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.dianyingLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`dianyingLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`dianyingLiuyan`, this.ruleForm);
                }
