	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dianyingLiuyanId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dianyingLiuyan', this.dianyingLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dianyingLiuyanPhoto ? this.detail.dianyingLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dianyingLiuyanYuyue', {
									yonghuId: _this.user.id,
									dianyingLiuyanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/dianyingLiuyanYuyue/list');
							}
						}
					});
				},
